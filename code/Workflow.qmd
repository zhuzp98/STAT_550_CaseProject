...
```{r}
# data <- read_excel("Stat_Data.xlsx", skip = 1)
...
c1 <- c1 %>% dplyr::filter(!is.na(Total_3M) | !is.na(Total_6M) | !is.na(Total_1Y) | !is.na(Total_5Y)) %>%
  dplyr::filter(Total_3M != 0 | Total_6M != 0 | Total_1Y != 0 | Total_5Y != 0)
```
...
c3 <- c1 %>% dplyr::filter((1*(is.na(Total_3M)) + 1*(is.na(Total_6M)) + 1*(is.na(Total_1Y)) + 1*(is.na(Total_5Y))) <= 2)
```
